...
def test_get_exception_occurrence(session, exception_occurrence: ExceptionOccurrence):
    e_info = get_exception_occurrence(session, exception_occurrence.request_id)
    ...
    assert e_info.stack_trace_snapshot_id == exception_occurrence.stack_trace_snapshot_id
